...
exports.TOOL_DECLARATIONS = [
    {
        ...
        description: "Retrieve formatted, LLM-ready context from the ESKOS Knowledge Fabric for a natural-language question. Use this first for any product/technical question.",
        parameters: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Run a structured hybrid query (vector + graph) against the Knowledge Fabric when you need raw hits and graph relationships, not just prose context.",
        parameters: {
            type: "object",
            properties: {
                query: { type: "string" },
    ...
    },
    {
        ...
        description: "Submit a proposed content draft for HUMAN REVIEW. This does NOT publish anything. Use this when the user asks you to draft/write content, not when just answering a question.",
        parameters: {
            type: "object",
            properties: {
                title: { type: "string" },
...
async function callKnowledgeFabric(path, body) {
    const { data } = await axios_1.default.post(`${KONG_BASE_URL}${path}`, body, { timeout: 10000 });
    return data;
}
async function callKnowledgeFabricGet(path, params) {
    const { data } = await axios_1.default.get(`${KONG_BASE_URL}${path}`, { params, timeout: 10000 });
    return data;
}
async function callTrustScore(sourceCategory, orgId) {
    const { data } = await axios_1.default.post(`${KONG_BASE_URL}/api/v1/knowledge/trust-score`, { source_category: sourceCategory, org_id: orgId }, { timeout: 10000 });
    return data;
}
async function submitGovernanceDraft(args, ctx) {
    const { data } = await axios_1.default.post(`${KONG_BASE_URL}/api/v1/governance/drafts`, {
        title: args.title,
